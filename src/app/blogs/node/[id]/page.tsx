...
function getItemLink(item: ContentInterface): string {
    ...
    return `/blogs/${item.id}/${slug}`;
}

export default function BlogsNodePage() {
    ...
    useEffect(() => {
        ...
        async function fetchAndRedirect() {
            try {
                ...
                // اگر content_type_id مربوط به blogs نباشد یا محتوا وجود نداشته باشد
                if (content.content_type_id !== 62) {
                    // اگر محتوا blogs نباشد، به صفحه اصلی blogs redirect می‌کنیم
                    router.push('/blogs');
                    return;
                }

                // Redirect به مسیر کامل blogs
